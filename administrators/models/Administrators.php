...
class Administrator extends Connection
{
    ...
    {
        $stmt = $this->db->prepare("INSERT INTO `users` (name, lastname, user, password, profile, status) VALUES (:name, :lastname, :user, :password, 'administrator', 'active')");
    ...
    {
        $rows = null;
        $stmt = $this->db->prepare("SELECT * FROM users WHERE profile= 'administrator'");
        $stmt->execute();
    ...
    {
        $rows = null;
        $stmt = $this->db->prepare("SELECT * FROM users WHERE profile= 'administrator' AND id_user=:id");
